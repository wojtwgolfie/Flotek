        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            switch (result)
            {
                case SignInStatus.Success:
                    ...
                    {
                        return RedirectToAction("AdminPage", "Administrator");
                    }
                    else if (User.IsInRole("Dispatcher"))
                    {
                        return RedirectToAction("DispatcherPage", "Dispatcher");
                    }
                    else if (User.IsInRole("Worker"))
                    {
                        return RedirectToAction("WorkerPage", "Worker");
                    }
                    else
                    {
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    Profile profile = new Profile { UserName = model.Email, Imię = model.Imię, Nazwisko = model.Nazwisko, Miasto = model.Miasto, Zakład = model.Zakład };
                    ServicePlan servicePlan = new ServicePlan { UserName = model.Email };
